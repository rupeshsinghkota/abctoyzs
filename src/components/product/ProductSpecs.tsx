...
export function ProductSpecs({ specs, additionalInfo }: ProductSpecsProps) {
    ...
    return (
        <div className="h-full flex flex-col justify-between">
            <div className="grid grid-cols-2 gap-x-4 md:gap-x-12 gap-y-6">
                {items.map((item, idx) => (
                    <div key={idx} className="flex items-center gap-3 md:gap-5 group">
                        <div className="w-12 h-12 md:w-14 md:h-14 rounded-[16px] md:rounded-[20px] bg-gray-50 flex items-center justify-center text-gray-400 group-hover:bg-primary/5 group-hover:text-primary transition-all duration-300 shadow-sm border border-gray-100/50 shrink-0">
                            {item.icon && <item.icon className="w-5 h-5 md:w-6 md:h-6" strokeWidth={1.5} />}
                        </div>
                        <div className="flex flex-col gap-0.5">
                            <span className="text-[10px] md:text-[11px] uppercase tracking-widest font-bold text-gray-400 group-hover:text-primary/60 transition-colors truncate w-full">{item.label}</span>
                            <span className="text-base md:text-lg font-black text-gray-900 leading-none truncate w-full">{item.value}</span>
                        </div>
                    </div>
                ))}
            </div>
